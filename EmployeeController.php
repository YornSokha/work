...
if ($_POST["form_state"] == "create_employee"){
    ...
    if (isset($_POST["btn_ok"])){
        ...
        if (!$result){
            ...
            $errors = '<p style="border: 1px solid red; font-size: 18px; color: red;">Error select employee</p>';
            include ("views/error.php");
            unset($hidden_datas);
            exit;
        }
        ...
        $errors = validate($validate_datas);
        if($errors != ""){
            $hidden_datas = ["id" => $id, "company_id" => $company_id, "employee_name" => $employee_name, "employee_surname" => $employee_surname, "employee_telephone" => $telephone, "employee_salary" => $salary];
            $action = "EmployeeController.php";
            include ("views/error.php");
            unset($hidden_datas);
            exit;
        ...
        if(!$result){
            $hidden_datas = ["id" => $id, "company_id" => $company_id,
                "employee_name" => $employee_name,
                "employee_surname" => $employee_surname,
                "employee_telephone" => $telephone,
                "employee_salary" => $salary];
