	...
	<form action="CompanyController.php" method="post">
		...
		<table>
			<tbody>
				<tr>
					<td>Company Name:</td>
					<td><input type="text" name="company_name" value="<?php echo isset($_POST["company_name"]) ? htmlentities($_POST['company_name']) : '' ?>"></td>
				</tr><tr>
					<td>License:</td>
					<td><input type="text" name="company_license" value="<?php echo isset($_POST["company_license"]) ? htmlentities($_POST['company_license']) : '' ?>"></td>
				</tr><tr>
					<td>Address:</td>
					<td><input type="text" name="company_address" value="<?php echo isset($_POST["company_address"]) ? htmlentities($_POST['company_address']) : '' ?>"></td>
				</tr>
			</tbody>
		</table>
		...
		<table class="table">
            ...
            <?php foreach ($companies as $company){ ?>
                <tr>
                    ...
                    <td><?php echo htmlentities($company["license_no"]) ?></td>
                    <td><a href="#" onclick="goto_modify_page_by_id(<?php echo htmlentities($company["id"]) ?>, 1, 'form_list_company')">Edit</a></td>
                    <input type="hidden" name="company_id" value="<?php echo htmlentities($company["id"]) ?>">
                </tr>  
            <?php } ?>
            </tbody>
		</table>
                ...
                <table class="table">
                    ...
                    <?php foreach ($employees as $employee){ ?>
                        <tr>
                            ...
                            <td><?php echo htmlentities($employee["surname"]) ?></td>
                            <td><?php echo htmlentities($employee["telephone"]) ?></td>
                            <td><?php echo "$". htmlentities($employee["salary"]) ?></td>
                        </tr>
                    <?php }?>
                    </tbody>
                </table>
